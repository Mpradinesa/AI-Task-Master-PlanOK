...
// Definimos las categorías, estados y prioridades con los valores técnicos que Django espera
...
export function TaskFormDialog({ open, task, onClose, onSubmit, isSubmitting }: TaskFormDialogProps) {
  ...
  const handleSubmit = () => {
    ...
    const taskData = {
      ...
      status: STATUS_MAP[status] || 'pending', // Convertimos a inglés para Django
      priority: 'medium', // Valor por defecto técnico
      subTasks,
      due_date: dueDate || null,
    };

    if (isEditing && task) {
      onSubmit({ id: task.id, data: taskData as UpdateTaskDto });
    } else {
      onSubmit(taskData as CreateTaskDto);
    }
  };
